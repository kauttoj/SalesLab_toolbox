...
# FUNCTIONS
def add_box():
	# add lines and pixel count
	yy = pyplot.gca().get_ylim()
	xx = pyplot.gca().get_xlim()
	pyplot.plot((xx[0],xx[0]),yy,'k',linewidth=2)
	pyplot.plot((xx[1],xx[1]),yy, 'k', linewidth=2)
	pyplot.plot(xx,(yy[0],yy[0]), 'k', linewidth=2)
	pyplot.plot(xx,(yy[1],yy[1]), 'k', linewidth=2)
	pyplot.text(xx[1]-10,yy[1]-10,'(x=%i,y=%i)' % (xx[1],yy[0]),horizontalalignment='right',verticalalignment='bottom',fontsize=18)
...
def draw_fixations(fixations, dispsize, imagefile=None, durationsize=True, durationcolour=True, alpha=0.5, savefilename=None):
	...
	if savefilename != None:
		fig.savefig(savefilename + '.png',bbox_inches="tight")
		fig.savefig(savefilename + '.pdf', bbox_inches="tight")
...
def draw_display(dispsize, imagefile=None):
	...
	screen = 255*numpy.ones((dispsize[1],dispsize[0],3), dtype=data_type)
